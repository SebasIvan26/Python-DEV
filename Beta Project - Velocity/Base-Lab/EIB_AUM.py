...
def getAUMTemplate(source):
    url = "https://github.com/SebasIvan26/testinghello/blob/master/AUM%20EIB%20Template.xlsx?raw=true"
    ...
    for row in ws.iter_rows(min_row=row_loc, max_row=16, min_col=1, max_col=46, values_only=True):
        if row[4]:
            ...
            if 'GLOBAL_INTL' in ex_ref_ID and 'DOMESTIC' in bu_Data_Audit[BU] and '79997' in ledger:
                ...
                ws.cell(row=row_loc, column=16).value = getMonthYear() + " STAT AUM - OP IVT"
            if '79993' in ledger:
...
def main(dic, bucketDestPath):
    ...
    bucketDestPath = bucketDestPath +'_EIBUpload.xlsx' if '.xlsx' not in bucketDestPath else bucketDestPath.replace('.xlsx', '_EIBUpload.xlsx')
